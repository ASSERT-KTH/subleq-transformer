...
The 45 failures in Tier~5 are from error accumulation over 200 autoregressive steps on random programs---since the model is applied autoregressively, any per-step rounding imprecision in floating-point arithmetic compounds over many steps.
...
At inference time, the predicted output is fed back as input, enabling autoregressive multi-step execution.
...
\begin{table}[h]
\centering
\caption{Multi-step program execution accuracy. Programs marked with $\dagger$ were never present in the training data in any form---these are emergent capabilities. Steps indicates the maximum number of autoregressive model invocations required.}
\label{tab:multistep}
...
For autoregressive systems, per-step accuracy in the 99.9\%+ regime is qualitatively different from 99.8\%---it is the threshold at which long programs become reliably executable.
\end{observation}
...
\end{tikzpicture}
\caption{Complete multiplication table computed by the model via autoregressive SUBLEQ execution. Each entry requires $\sim$3$b$ steps of autoregressive inference. This program was never in the training data.}
\label{fig:multiply}
\end{figure}
...
The longest computation (126 $\div$ 7 = 18) requires 91 autoregressive steps---91 consecutive correct SUBLEQ step predictions with zero errors.
...
and (3)~correct execution over long horizons (up to 91 autoregressive steps).
...
Additionally, the programs tested here are relatively short; it remains open whether the model would maintain reliability over thousands of autoregressive steps.
